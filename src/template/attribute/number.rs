use crate::{
    log_debug, log_debug_mount_end, log_debug_mount_start, spawn_on_command, Container, SiAttServer,
};
use crate::{Error, StableNumber};
...
pub trait NumberAccessorModel: Sync + Send {
    ///
    ///
    async fn get_number_at(&mut self, index: usize) -> Result<StableNumber, Error>;
    ///
    ///
    async fn set_number_at(&mut self, index: usize, value: &StableNumber) -> Result<(), Error>;
}
...
>(
    ...
    //
    // Just init
    let value = interface.lock().await.get_number_at(index).await?;
...
async fn on_command<I: NumberAccessorModel + 'static>(
    ...
    while let Some(command) = att.pop_cmd().await {
        ...
        //
        // Read back
        let read_back_value = interface.lock().await.get_number_at(index).await?;
